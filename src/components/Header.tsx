      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="hidden md:flex justify-center items-center h-16 px-6">
          <Link to="/" className="flex items-center gap-3" aria-label="Go to homepage">
            <img src="/logo.png" alt="MONAARC Logo" className="h-10 w-auto object-contain" draggable={false} />
        ...
        <div className="hidden md:grid grid-cols-3 items-center h-14 px-6">
          ...
          <div className="flex items-center">
            <Link to="/cart" className="relative text-white hover:text-yellow-400 transition" aria-label="Cart">
              ...
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 bg-yellow-400 text-black text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold" aria-label={`${totalItems} items in cart`}>
                  {totalItems}
                </span>
              )}
            </Link>
        ...
        {/* ===== MOBILE MENU (drawer) ===== */}
        {isMenuOpen && (
          <div id="mobile-menu" className="md:hidden px-4 pb-3 pt-2 bg-black/85 backdrop-blur border-t border-white/10">
            <nav className="space-y-1">
              <Link to="/" className="block px-2 py-2 text-white hover:text-yellow-400" onClick={() => setIsMenuOpen(false)}>
                Home
              </Link>
              ...
              <Link to="/new-arrivals" className="block px-2 py-2 text-white hover:text-yellow-400" onClick={() => setIsMenuOpen(false)}>
                New Arrivals
              </Link>
              <Link to="/about" className="block px-2 py-2 text-white hover:text-yellow-400" onClick={() => setIsMenuOpen(false)}>
                About
              </Link>
